...
I want to try something new here so this article uses a sort of **inverted pedagogy**: code first, explanation second.



### Each example is organized as follow:
1. **Real-world context:** Few words about the context
1. **Runnable Example:**
...
1. **Find More Examples:** don't trust me. Check by yourself. Search for real use case in crates, not ad hoc projects nor toy project (see below how to)
...
The idea is to learn from others and "see" how do they do. To do so I propose to clone the projects below.
...
```
Other ideas include but are not limited to : bat, fd, tokei, hyperfine...
...
Above we look into the codebase of `hexyl`. This wok the same way for `zoxyde` or `turbo-vision-4-rust`.
