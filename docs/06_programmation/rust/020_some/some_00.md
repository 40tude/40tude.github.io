...
* A function (or a method) that doesn’t always have a meaningful value to return should return an `Option<T>`. Respectively a `struct` whose fields may be empty should be of type `Option<T>`.
